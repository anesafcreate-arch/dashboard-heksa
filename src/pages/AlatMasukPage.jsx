...
export default function AlatMasukPage() {
  ...
  const resetForm = () => {
    setFormData({ kodeAlat: '', namaAlat: '', jenisLayanan: '' });
    setFile(null);
    setFileError('');
    setEditingItem(null);
  };
  ...
  const handleSubmit = async () => {
    if (!formData.kodeAlat || !formData.namaAlat || !formData.jenisLayanan) return;

    if (editingItem) {
      editAlatMasuk({ id: editingItem.id, ...formData });
    } else {
      const newItem = {
        ...formData,
      ...
      // Broadcast realtime ke semua user agar notifikasi + suara sinkron
      try {
        await supabase.channel('global-notif').send({
          ...
          payload: {
            ...
            tanggalMasuk: newItem.tanggalMasuk,
          },
        });
      } catch {
